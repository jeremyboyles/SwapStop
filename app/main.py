...
# serve frontend files (so chat.html is available from same origin)
...
async def websocket_endpoint(websocket: WebSocket, room_id: str, username: str):
    ...
    try:
        await manager.broadcast(room_id, f"__join__:{username}")
        while True:
            data = await websocket.receive_text()
            await manager.broadcast(room_id, f"{username}: {data}")
    except WebSocketDisconnect:
        manager.disconnect(room_id, websocket)
        await manager.broadcast(room_id, f"__leave__:{username}")
